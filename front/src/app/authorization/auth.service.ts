...
export class AuthService {
  ...
  login(username: string, password: string){
    this.http.post<{token: string}>("http://localhost:8080/login/login", {username: username, password: password}).subscribe(response =>{
      if(response.token){
  ...
  logout(){
    ...
    this.snackBarService.openSnackBar("See ya " + "!", "X")
  }
